...
Rscript -e "source('https://bioconductor.org/biocLite.R'); pkgs <- c('Biostrings', 'GenomicRanges', 'GenomicFeatures', 'org.Hs.eg.db', 'biomaRt', 'BSgenome.Hsapiens.UCSC.hg19', 'BSgenome.Hsapiens.UCSC.hg38', 'org.Mm.eg.db', 'BSgenome.Mmusculus.UCSC.mm10', 'org.Rn.eg.db', 'BSgenome.Rnorvegicus.UCSC.rn6', 'derfinder', 'bumphunter', 'LieberInstitute/jaffelab', 'devtools', 'getopt', 'BiocParallel', 'rafalib', 'SummarizedExperiment', 'plyr', 'rtracklayer', 'RColorBrewer'); present <- installed.packages(); biocLite(pkgs[!gsub('.*/', '', pkgs) %in% rownames(present)]); options(width = 120); devtools::session_info()"
