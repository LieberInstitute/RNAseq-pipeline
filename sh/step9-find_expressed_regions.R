...
save(region_cuts,
    file = file.path(opt$outdir, paste0('region_cuts_', strand, '.Rdata')))
...
regInfo <- data.frame(cutoff = cuts, n = n, mean = mean, sd = sd,
    stringsAsFactors = FALSE)
regInfo <- cbind(regInfo, quantile)
save(regInfo,
    file = file.path(opt$outdir, paste0('region_info_', strand, '.Rdata')))

## Make plots exploring the relationship between the cutoff and the number of regions
pdf(file = file.path(opt$outdir, paste0('region_info_', strand, '.pdf')),
    width = 10, height = 10)
